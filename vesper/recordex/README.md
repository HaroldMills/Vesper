...
* Change your Raspberry Pi's hostname (if you wish) in `Preferences -> Control Centre -> System`. The default hostname is `raspberrypi`.

* Enable SSH for your Raspberry Pi in `Preferences -> Control Centre -> Interfaces`.
...
* Run the installer in a terminal in the download directory with `bash <installer file name>`. Accept the defaults for the installation, including installation in the `~/miniforge3` directory. After the installation, the installer will offer to update your shell profile to automatically initialize `conda`. Accept the offer.
