...
RFFT.prototype.forward = function(buffer) {
  ...
  */
  ...
  while (--i) {
    rval = x[i];
    ival = x[n-i-1]; // This is incorrect: n-i-1 should be n-i.
    mag = bSi * sqrt(rval * rval + ival * ival);
    // Remove the following when the above bug is fixed.
    for (let ii = 0; ii != n; ++ii)
    	console.log(ii, x[ii]);
    console.log("hello", i, rval, n - i - 1, ival, mag);
  ...
  // There's another bug here: x[0] should be Math.abs(x[0]).
  // This code also discards the fs/2 bin.
  spectrum[0] = bSi * x[0];
