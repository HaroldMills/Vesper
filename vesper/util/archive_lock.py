...
"database is locked". Proper use of the archive lock prevents
these errors by ensuring that only one transaction is attempted
at a time.

To use the lock, any code that writes to an archive should run
inside a Python `with` statement whose context manager is the lock.
...
SQLite archive database sometimes fail if more than one thread and/or
process attempts to write to the database at once. For example, we have
seen this problem when trying to classify all of the clips of a clip
album page with a single keystroke (such a keystroke used to initiate
multiple annotation requests to the server: now it initiates only one),
and even when trying to classify multiple clips with one keystroke per
clip on a slow computer. Even though each clip classification was
attempted on the server in its own database transaction, assuring
atomicity, many classification requests (one for each clip) went to
the server in quick succession, resulting in multiple concurrent
transactions, some of which failed. A Django `OperationalError`
...
rather that it does not support concurrent transactions at all,
at least as configured here.
"""
