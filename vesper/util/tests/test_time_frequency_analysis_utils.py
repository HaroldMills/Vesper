...
class TimeFrequencyAnalysisUtilsTests(TestCase):
    ...
    def test_linear_to_log(self):
        ...
        ref_power = 10
        ref_power_db = 10 * np.log10(ref_power)
        for spectra, expected in cases:
            spectra = np.array(spectra, dtype='float64')
            expected = np.array(expected, dtype='float64')
            expected[expected != minus_infinity] -= ref_power_db
            self._test_op(
                expected, tfa_utils.linear_to_log, spectra, ref_power)
    ...
    def test_log_to_linear(self):
        ...
        for spectra, expected in cases:
            ...
            expected *= ref_power
            self._test_op(
                expected, tfa_utils.log_to_linear, spectra, ref_power)
